...
def nome_sito(request):
    ...
    )  # Scegli la Facility corretta o utilizza la logica di scelta desiderata
    return {
        "nome_sito": facility.nome_sito if facility else "",
        "indirizzo": facility.indirizzo if facility else "",
        "cap": facility.cap if facility else "",
        "city": facility.city if facility else "",
        "provincia": facility.provincia if facility else "",
        "country": (
            facility.country.name if facility and facility.country else ""
        ),  # Assicurati che il campo `country` sia un oggetto `Country` di `django-countries`
    }
...
def fk_ward_records(request):
    ...
    ward_records = [("tutti", "Tutti")] + [
        (ward.pk, ward.description) for ward in wards
    ]
