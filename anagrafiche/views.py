...
'''class UpdateSupplier(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["listini_terzisti"] = ListinoTerzista.objects.filter(
            fk_fornitore=self.object.pk
        )
        context["gestori_rifiuti"] = XrDocumentiGestore.objects.filter(
            fornitore_rifiuti=self.object.pk
        )
        context["smaltitori_rifiuti"] = XrDocumentiSmaltitore.objects.filter(
            fornitore_rifiuti=self.object.pk
        )
        context["trasportatori_rifiuti"] = XrDocumentiTrasportatore.objects.filter(
            fornitore_rifiuti=self.object.pk
        )
        return context
...
class ListinoTerzistaCreateView(CreateView):
    ...
    def get_success_url(self):
        ...
        if "salva_esci" in self.request.POST:
            ...
            return reverse_lazy(
                "anagrafiche:vedi_fornitore", kwargs={"pk": pk_fornitore}
            )
...
class ListinoTerzistaUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["fornitore"] = Fornitore.objects.get(pk=fk_fornitore)
        context["prezzi"] = PrezzoListino.objects.filter(
            fk_listino_terzista=self.object.pk
        )
...
class PrezzoListinoCreateView(CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "anagrafiche:modifica_voce_listino", kwargs={"pk": fk_listino_terzista}
        )
...
class XrDocumentiGestoreCreateView(CreateView):
    ...
    def get_initial(self):
        ...
        fornitore_rifiuti = get_object_or_404(
            FornitoreRifiuti, fornitore_ptr_id=self.kwargs["fk_fornitore"]
        )
    ...
    def form_valid(self, form):
        ...
        form.instance.fornitore_rifiuti = get_object_or_404(
            FornitoreRifiuti, fornitore_ptr=self.kwargs["fk_fornitore"]
        )
        return super().form_valid(form)
...
def delete_gestore_rifiuti(request, pk):
    ...
    deleteobject.delete()
    url_match = reverse_lazy(
        "anagrafiche:delete_gestore_rifiuti", kwargs={"pk": fornitore}
    )
    return redirect(url_match)
...
class ListinoClienteCreateView(CreateView):
    ...
    def get_success_url(self):
        ...
        if "salva_esci" in self.request.POST:
            ...
            return reverse_lazy(
                "anagrafiche:modifica_cliente", kwargs={"pk": pk_cliente}
            )

        pk = self.object.pk
        return reverse_lazy(
            "anagrafiche:modifica_voce_listino_cliente", kwargs={"pk": pk}
        )
