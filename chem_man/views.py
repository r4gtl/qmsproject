...
from .models import (
    ...
    DettaglioAcquistoProdottoChimico,
    DettaglioOrdineProdottoChimico,
    HazardStatement,
    HazardStatement_SDS,
    ImballaggioPC,
    OrdineProdottoChimico,
    PrecautionaryStatement,
    PrecautionaryStatement_SDS,
    PrezzoProdotto,
    ProdottoChimico,
    SchedaSicurezza,
    SchedaTecnica,
...
def home_prodotti_chimici(request):
    ...
    prodotti_chimici_filter = ProdottoChimicoFilter(
        request.GET, queryset=prodotti_chimici
    )
    ultimo_agg_svhc = SostanzaSVHC.objects.aggregate(max_date=Max("data_inclusione"))[
        "max_date"
    ]
    page = request.GET.get("page", 1)
    paginator = Paginator(
        prodotti_chimici_filter.qs, 50
    ...
    totale_solvente = acquisti_anno_corrente.aggregate(
        totale_solvente=Sum(
            F("dettagli_acquisto__quantity") * F("dettagli_acquisto__solvente")
        )
        / 100
    )["totale_solvente"]
...
class ProdottoChimicoCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "chem_man:modifica_prodotto_chimico", kwargs={"pk": pk_prodotto_chimico}
        )
...
class ProdottoChimicoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["elenco_prezzi"] = PrezzoProdotto.objects.filter(
            fk_prodottochimico=pk_prodottochimico
        )
        context["elenco_schede_tecniche"] = SchedaTecnica.objects.filter(
            fk_prodottochimico=pk_prodottochimico
        )
        context["elenco_schede_sicurezza"] = SchedaSicurezza.objects.filter(
            fk_prodottochimico=pk_prodottochimico
        )
...
class PrezzoProdottoCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "chem_man:modifica_prodotto_chimico_with_focus_button",
            kwargs={"pk": fk_prodottochimico, "focus_button": focus_button},
        )
    ...
    def get_initial(self):
        ...
        return {"created_by": created_by, "fk_prodottochimico": fk_prodottochimico}

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        fk_prodottochimico = self.kwargs["fk_prodottochimico"]
        context["fk_prodottochimico"] = ProdottoChimico.objects.get(
            pk=fk_prodottochimico
        )
...
class PrezzoProdottoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        fk_prodottochimico = self.object.fk_prodottochimico.pk
        return reverse_lazy(
            "chem_man:modifica_prodotto_chimico", kwargs={"pk": fk_prodottochimico}
        )
...
def tabelle_generiche(request):
    ...
    hazard_statements_filter = HazardStatementFilter(
        request.GET, queryset=hazard_statements
    )
    filtered_hazard_statements = hazard_statements_filter.qs
    hazard_statements_filter_count = filtered_hazard_statements.count()

    precautionary_statements_filter = PrecautionaryStatementFilter(
        request.GET, queryset=precautionary_statements
    )
    ...
    try:
        ...
        sostanze_svhc_paginator = paginator_sostanze_svhc.page(
            paginator_sostanze_svhc.num_pages
        )
    ...
    try:
        hazard_statements_paginator = paginator_hazard_statements.page(
            page_hazard_statements
        )
    except PageNotAnInteger:
        hazard_statements_paginator = paginator_hazard_statements.page(1)
    except EmptyPage:
        hazard_statements_paginator = paginator_hazard_statements.page(
            paginator_hazard_statements.num_pages
        )

    # Paginazione Precautionary Statements
    page_precautionary_statements = request.GET.get("page_precautionary_statements", 1)
    paginator_precautionary_statements = Paginator(
        filtered_precautionary_statements, 50
    )
    try:
        precautionary_statements_paginator = paginator_precautionary_statements.page(
            page_precautionary_statements
        )
    except PageNotAnInteger:
        precautionary_statements_paginator = paginator_precautionary_statements.page(1)
    except EmptyPage:
        precautionary_statements_paginator = paginator_precautionary_statements.page(
            paginator_precautionary_statements.num_pages
        )
    ...
    try:
        ...
        simboli_ghs_paginator = paginator_simboli_ghs.page(
            paginator_simboli_ghs.num_pages
        )
    ...
    try:
        ...
        imballaggi_pc_paginator = imballaggi_pc_paginator.page(
            imballaggi_pc_paginator.num_pages
        )
...
class SchedaSicurezzaCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        fk_prodottochimico = self.object.fk_prodottochimico.pk
        return reverse_lazy(
            "chem_man:modifica_scheda_sicurezza",
            kwargs={"fk_prodottochimico": fk_prodottochimico, "pk": pk},
        )
...
class SchedaSicurezzaUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "chem_man:modifica_scheda_sicurezza",
            kwargs={"fk_prodottochimico": fk_prodottochimico, "pk": pk_sds},
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["elenco_hazard_statements"] = HazardStatement_SDS.objects.filter(
            fk_sds=pk_sds
        )
        context["elenco_precautionary_statements"] = (
            PrecautionaryStatement_SDS.objects.filter(fk_sds=pk_sds)
        )
...
class SimboloGHS_SDSCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "chem_man:modifica_scheda_sicurezza_with_focus_button",
            kwargs={
    ...
    def get_initial(self):
        ...
        return {"created_by": created_by, "fk_sds": fk_sds}
...
class HazardStatement_SDSCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        fk_prodottochimico = self.object.fk_sds.fk_prodottochimico.pk
        focus_button = (
            "btn_hazard_statement"  # Imposto il pulsante su cui settare il focus
        )
        success_url = reverse_lazy(
            "chem_man:modifica_scheda_sicurezza_with_focus_button",
            kwargs={
...
def delete_hazard_statement_sds(request, pk):
    ...
    url_match = reverse_lazy(
        "chem_man:modifica_scheda_sicurezza",
        kwargs={"fk_prodottochimico": fk_prodottochimico, "pk": fk_sds},
    )
    return redirect(url_match)
...
class PrecautionaryStatement_SDSCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        fk_prodottochimico = self.object.fk_sds.fk_prodottochimico.pk
        focus_button = (
            "btn_precautionary_statement"  # Imposto il pulsante su cui settare il focus
        )
        success_url = reverse_lazy(
            "chem_man:modifica_scheda_sicurezza_with_focus_button",
            kwargs={
...
class Sostanza_SDSCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["fk_sds"] = SchedaSicurezza.objects.get(pk=pk)
        context["fk_prodottochimico"] = sds.fk_prodottochimico
        context["filter"] = SostanzaFilter(
            data=self.request.GET, queryset=Sostanza.objects.all()
        )
...
def dashboard_acquisti_prodotti_chimici(request):
    ...
    return render(
        request, "chem_man/acquisti/home_acquisti_prodotti_chimici.html", context
    )
...
def home_ordini_prodotti_chimici(request):
    ...
    return render(
        request, "chem_man/acquisti/home_ordini_prodotti_chimici.html", context
    )
...
class OrdineProdottoChimicoCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "chem_man:modifica_ordine_prodotto_chimico", kwargs={"pk": pk_ordine}
        )
    ...
    def calculate_initial_numero_ordine(self):
        ...
        max_value = OrdineProdottoChimico.objects.filter(
            created_at__year=current_year
        ).aggregate(Max("numero_ordine"))["numero_ordine__max"]
...
class OrdineProdottoChimicoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # context['elenco_prezzi'] = PrezzoProdotto.objects.filter(fk_prodottochimico=pk_prodottochimico)
        # context['elenco_schede_tecniche'] = SchedaTecnica.objects.filter(fk_prodottochimico=pk_prodottochimico)
        context["elenco_dettagli"] = DettaglioOrdineProdottoChimico.objects.filter(
            fk_ordine=pk_ordine
        )
...
class DettaglioOrdineProdottoChimicoCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        success_url = reverse_lazy(
            "chem_man:modifica_ordine_prodotto_chimico_with_focus",
            kwargs={"pk": fk_ordine, "focus_button": focus_button},
        )
        return success_url
    ...
    def get_initial(self):
        ...
        return {"created_by": created_by, "fk_ordine": fk_ordine}
...
class DettaglioOrdineProdottoChimicoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["dettagli_ordine"] = get_object_or_404(
            OrdineProdottoChimico, pk=fk_ordine
        )
        # context['elenco_schede_tecniche'] = SchedaTecnica.objects.filter(fk_prodottochimico=pk_prodottochimico)
...
def delete_dettaglio_ordine_prodotto_chimico(request, pk):
    ...
    deleteobject.delete()
    url_match = reverse_lazy(
        "chem_man:modifica_ordine_prodotto_chimico", kwargs={"pk": fk_ordine}
    )
    return redirect(url_match)
...
def home_acquisti_prodotti_chimici(request):
    ...
    acquisti_pc_filter = AcquistoProdottoChimicoFilter(
        request.GET, queryset=acquisti_pc
    )

    page = request.GET.get("page", 1)
    paginator = Paginator(
        acquisti_pc_filter.qs, 50
    )  # Utilizza fornitori_filter.qs per la paginazione
...
class AcquistoProdottoChimicoCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "chem_man:modifica_acquisto_prodotto_chimico", kwargs={"pk": pk_acquisto}
        )
...
class AcquistoProdottoChimicoUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        pk_acquisto = self.object.pk
        context["elenco_dettagli"] = DettaglioAcquistoProdottoChimico.objects.filter(
            fk_acquisto=pk_acquisto
        )
        # Calcola la somma della quantità dei dettagli di acquisto
        somma_quantita = DettaglioAcquistoProdottoChimico.objects.filter(
            fk_acquisto=pk_acquisto
        ).aggregate(somma_quantita=Sum("quantity"))
...
class DettaglioAcquistoProdottoChimicoCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields["fk_prodotto_chimico"].queryset = form.fields[
            "fk_prodotto_chimico"
        ].queryset.filter(fk_fornitore=acquisto.fk_fornitore)
        return form
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "chem_man:modifica_acquisto_prodotto_chimico", kwargs={"pk": fk_acquisto}
        )
    ...
    def get_initial(self):
        ...
        return {"created_by": created_by, "fk_acquisto": fk_acquisto}
...
def stampa_ordine(request, pk):
    ...
    context = {"ordine": ordine, "dettagli_ordine": dettagli_ordine}
...
def controlla_dettagli_ordine(request, pk):
    ordine = get_object_or_404(OrdineProdottoChimico, id=pk)
    dettagli_presenti = DettaglioOrdineProdottoChimico.objects.filter(
        fk_ordine=ordine
    ).exists()
