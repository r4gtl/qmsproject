...
def ricetta_colore_bagnato_print(request, pk):
    ricetta = get_object_or_404(RicettaColoreBagnato, pk=pk)
    dettagli_ricetta = DettaglioRicettaColoreBagnato.objects.filter(fk_ricetta_colore_bagnato=pk)
    # Sottquery per ottenere l'ultimo prezzo per ciascun prodotto chimico
    ultimi_prezzi = PrezzoProdotto.objects.filter(
        fk_prodottochimico=OuterRef('fk_prodotto_chimico')
    ).order_by('-data_inserimento').values('prezzo')[:1]
    ...
    return render(request, 'ricette/reports/ricetta_colore_bagnato_print.html', context)